...
if(preg_match("/postparent\/([0-9]+)/", $url, $matches) && $_SERVER['REQUEST_METHOD'] == 'PUT'){
    error_log("Modify parent");
    $id = $matches[1];
    error_log($id);
    $url_components = parse_url($url);
    parse_str($url_components['query'], $params);
    $distro_values[0] = $params["parentname"];
    if("" == trim($distro_values[0])){//Si esta vacio, N/A
        $distro_values[0] = "N/A";
    }
    error_log($distro_values[0]);
    


    $post = EditParent($distro_values, $id, $dbConn);
...
function EditParent($distro_values, $id, $db){
    $statement = $db->prepare("
    UPDATE UNIX.FAMILIA
	SET NOM_FAMILIA=:parentname
	WHERE ID_FAMILIA=:id;");

    $statement->bindValue(':id', $id);
    $statement->bindValue(':parentname', $distro_values[0]);
